...
def execute(method,*args):
    ...
    job_defaults = {'max_instances': 3} #设置job同时运行，防止job运行时间大于调度周期时间，当设置为5时最好job不要访问全局变量
    ...
    try:
        ...
        scheduler.add_job(method, 'cron', args=[*args], year=year,month=month,week=week,day=day, day_of_week=day_of_week,minute=minute,second=second,start_date=start_date, end_date=end_date)
        # scheduler.add_job(method, 'cron', year=year,month=month,week=week,day=day, day_of_week=day_of_week,minute=minute,second=second,start_date=start_date, end_date=end_date)
